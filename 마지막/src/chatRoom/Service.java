...
private Server server;	// 클라이언트의 정보(추가, 클라이언트 삭제, 모든 메시지 관리)
private Socket socket;

	String UserName = "ha"; // 클라이언트 이름
	...
	public void run()	// 클라이언트 창에 출력하는 코드를 작성
	{
		try
		{
			...
			System.out.println("# " + UserName + " 님이 들어오셨습니다");
		...
		{
			server.removeClient(this); // 클라이언트 삭제, 데이터베이스 업데이트
			server.sendMessageAll("# " + UserName + " 님이 나가셨습니다");
