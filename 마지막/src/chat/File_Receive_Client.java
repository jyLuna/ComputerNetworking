...
public class File_Receive_Client extends JFrame implements Runnable{
	private JTextArea msgView=new JTextArea();     // 파일을 보여주는 텍스트영역
	private JTextField sendBox=new JTextField();   // 파일이름을 입력하는 텍스트필드
	...
	public File_Receive_Client(String ip, int port) throws IOException {
			...
			try{
				while( (line = reader.readLine()) != null ){
					...
					if(line.startsWith("[NAME]")){
						...
						Save_File.filename.setText(name);			//파일 이름 설정.
						Thread.sleep(200);
						String str_size = parsingData[2];
						Save_File.allsize.setText(str_size);			//파일 크기 설정.
 						try{
						this.size = Integer.parseInt(str_size);
						Save_File.statusBar.setMaximum(Integer.parseInt(str_size)/1024);			//상태바의 최대 크기.
						Save_File.statusBar.setMinimum(0);					//상태바의 최소 크기
	 ...
	 *-----------------------------------------------------*/
			...
			if( bin != null){
				...
				while( (temp = inst.read(buffer) ) != -1 ){
					...
					Save_File.size.setText(String.valueOf(size));		//화면에 size(전송량)을 설정.
					Save_File.statusBar.setValue(Save_File.statusBar.getValue()+1);		//상태바 +1
					out.write(buffer);
				}
				//다 받았을때
				if( size >= Integer.parseInt(Save_File.allsize.getText()) ){
					Save_File.hideFrame();
				}
				/*	자 원 정 리	 */
				out.close();
		...
		private File makeFile(InputStream inst) {
			File tmpFile = null;
				String filePath = Save_File.path;
